...
export const SupervisorDashboard: React.FC<SupervisorDashboardProps> = ({ currentUser, onLogout }) => {
  ...
  useEffect(() => {
    const loadTeam = async () => {
      ...
      const { data: profiles } = await supabase
        .from('profiles')
        .select('*')
        .eq('empresa_id', currentUser.company)
  ...
  const calcularEquipoStats = async () => {
    ...
    // const users = JSON.parse(localStorage.getItem('users') || '[]'); -- REMOVIDO

    if (!currentUser.company || team.length === 0) return;

    // Fetch user progress from Supabase
    const progressData = await getCompanyProgress(currentUser.company);

    // Transformar user_progress a mapa: { username: { skillId: { ... } } }
    const newProgressMap: Record<string, any> = {};

    // Inicializar mapa para todos los miembros del equipo
    team.forEach(u => {
      newProgressMap[u.username] = {};
    });

    progressData.forEach((p: any) => {
      // Encontrar username por id (p.user_id)
      const user = team.find(t => t.id === p.user_id);
      if (user) {
        if (!newProgressMap[user.username]) newProgressMap[user.username] = {};
        newProgressMap[user.username][p.skill_id] = {
          status: p.status,
          xpEarned: 0, // Ajustar según esquema real si existe
    ...
    const participantes = team; // Ya están filtrados
    const equipoData = participantes.map((u: User) => {
      const progreso = newProgressMap[u.username] || {};
    ...
    participantes.forEach((p: User) => {
      const progreso = newProgressMap[p.username] || {};
  ...
  useEffect(() => {
    if (showTeamDashboard && currentUser.role === 'supervisor') {
      calcularEquipoStats();
    }
